...
{   
    ...
    PKGNAME=$pkgname su -c 'grep $PKGNAME /proc/mounts | while read -r line; do echo $line | cut -d " " -f 2 | sed "s/apk.*/apk/" | xargs -r umount -l > /dev/null 2>&1; done'
    revancedapp=/data/adb/revanced/$pkgname.apk
    su -mm -c "cp {$appname}Revanced-$appver.apk /data/local/tmp &&\
    mv /data/local/tmp/{$appname}Revanced-$appver.apk $revancedapp &&\
