...
changesource()
{
    ...
    selectsource=$("${header[@]}" --title 'Source Selection Menu' --keep-window --no-shadow --no-cancel --ok-label "Done" --radiolist "Select Source" $fullpageheight $fullpagewidth 10 "${selectedsolurce[@]}" 2>&1> /dev/tty)
...
checkmicrogpatch()
{
    ...
    then
        ...
        if [ "$microgstatus" = "on" ]
        then
            if "${header[@]}" --title 'MicroG Patch Prompt' --no-items --defaultno --keep-window --no-shadow --yesno --yes-label "Continue" --no-label "Exclude" "You have a rooted device and you have included a microg-support patch. This may result in YouTube app crash.\n\n\nDo you want to exclude it or continue?" $fullpageheight $fullpagewidth
            then
                return 0
            else
                tmp=$(mktemp)
...
mainmenu()
{
    ...
    mainmenu=$("${header[@]}" --title 'Select App' --keep-window --no-shadow --ok-label "Select" --cancel-label "Exit" --menu "Select Option" $fullpageheight $fullpagewidth 10 "${menuoptions[@]}" 2>&1> /dev/tty)
    exitstatus=$?
    if [ $exitstatus -eq 0 ]
    then
        if [ "$mainmenu" -eq "1" ]
